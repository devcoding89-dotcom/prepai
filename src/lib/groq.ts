...
function removeReasoning(text: string) {
  return text
...
export async function askGroq(opts: {
  ...
  const response = await fetch(API, {
    ...
    body: JSON.stringify({ model: MODEL, messages, temperature: 0.4, max_tokens: 800, reasoning_effort: "low" }),
    cache: "no-store",
  });
  ...
  if (!response.ok) throw new Error(payload.error?.message || "Groq could not answer right now.");

  const answer = removeReasoning(payload.choices?.[0]?.message?.content || "");
